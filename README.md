...
| 같은 방 사람들과 채팅하기 | 사무실 그림 왼쪽 아래에 늘 떠 있는 입력 칸에 적고 Enter 를 누르면 내 아바타 위에 말풍선이 뜨고 같은 방 사람들 화면에도 보입니다. 동료 방에서는 Enter 만 눌러도 바로 입력이 시작되고(`/` 키도 됩니다), 최근 대화는 사무실 아래에 줄로 남습니다. 방마다 따로이고 저장하지 않습니다. 이름은 별명이 있으면 별명, 없으면 팀 창의 내 이름입니다. |
