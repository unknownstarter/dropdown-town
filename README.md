...
| 새 일 시키기 | 오른쪽 위 "+ 새 세션"에서 프로젝트를 고르고 시킬 일을 적습니다. 최근에 세션이 돌았던 폴더와 이 맥의 다른 프로젝트 폴더(`~/Documents/GitHub`, `~/Projects` 등 바로 아래)가 목록에 뜨고, "직접 입력"으로 홈 폴더 아래 아무 경로나 적을 수도 있습니다. 릴레이도 같습니다. |
...
| 팀원의 사무실 구경하기 | 오른쪽 위 "팀". 둘 다 "공유 켜기"를 하면 같은 와이파이의 동료가 "근처 사무실"에 자동으로 나타납니다(맥의 Bonjour). "붙이기"를 누르면 상대 화면에 노크가 가고, 상대가 "허용"하면 서로 붙습니다. 주소와 키를 옮길 일이 없습니다. 다른 네트워크나 Tailscale 로 붙일 때만 "주소와 키를 직접 넣기"를 씁니다. 동료 방은 구경만 되고 세션 조작은 각자 자기 맥에서만 됩니다. 아바타가 문으로 걸어 나가면 옆 방으로 넘어갑니다. 동료 방에 들어가면 내 아바타가 동료 화면에도 보이고(☆ 표시), 내 사무실에 놀러 온 동료도 보입니다. 설정은 `~/.dropdown-town/config.json` 에 저장됩니다. |
| 이 방의 소식 창 | 큰 화면에서는 그림 오른쪽에, 작은 화면에서는 그림 아래에 "이 방의 소식과 대화" 칸이 있습니다. 세션이 지금 하는 일, 출근과 작업 끝, 호출 벨, 조수의 결과 넘김이 시간순으로 흐르고 그 사이에 사람 채팅이 섞입니다. 캐릭터를 누르면 큰 화면에서는 이 칸이 상태창으로 바뀌고, 닫으면(Esc) 소식 창으로 돌아옵니다. |
| 같은 방 사람들과 채팅하기 | 소식 창 아래 입력 칸에 적고 Enter 를 누르면 내 아바타 위에 말풍선이 뜨고 같은 방 사람들 화면에도 보입니다. 동료 방에서는 Enter 만 눌러도 바로 입력이 시작되고(`/` 키도 됩니다), 최근 대화는 사무실 아래에 줄로 남습니다. 방마다 따로이고 저장하지 않습니다. 이름은 별명이 있으면 별명, 없으면 팀 창의 내 이름입니다. |
| 내 캐릭터로 사무실 돌아다니기 | 별(★) 명찰을 단 캐릭터가 나입니다. 방향키나 WASD 로 걷고, 빈 바닥을 누르거나(휴대폰에서는 두드리거나) 그리로 걸어갑니다. 세션 캐릭터 옆에 서면 이름표가 켜지고 스페이스로 정보가 열리며, Esc 로 닫힙니다(입력 중이어도 됩니다). 위치와 꾸민 모습은 이 창에 저장됩니다. 세션 캐릭터는 위치가 곧 상태라서 손으로 옮길 수 없습니다. |
...
| 2.5 | 자율 사무실: 조수의 일과 말 보기, 결과 넘김 표시, 에이전트끼리 일을 이어 하는 릴레이, 세션끼리 대화하는 모습, 릴레이 중간 끼어들기 | 완료 |
...
| Start new work | "+ 새 세션" at the top right: pick a project and describe the task. The list shows folders where sessions ran recently plus other project folders on this Mac (directly under `~/Documents/GitHub`, `~/Projects` and similar), and "직접 입력" accepts any path under your home folder. Relays work the same way. |
...
| Visit a teammate's office | "팀" at the top right. When both of you turn on sharing, teammates on the same Wi-Fi show up under "근처 사무실" automatically (Bonjour). Press "붙이기" to knock; when they press "허용" you are connected both ways, no addresses or keys to copy. The manual address-and-key form is only for other networks or Tailscale. Teammate rooms are view-only; sessions can only be controlled on their own Mac. Walk your avatar out the door to move to the next room. When you enter a teammate's room your avatar shows up on their screen too (marked ☆), and teammates visiting your office appear in yours. Settings live in `~/.dropdown-town/config.json`. |
| Chat with people in the room | Type in the box under the office and press Enter: a speech bubble appears over your avatar and on everyone's screen in that room. In a teammate's room, plain Enter (or `/`) starts typing right away. Recent lines stay under the office, per room, and are not saved. Your name is your nickname if set, otherwise the name from the team dialog. |
| Walk around as yourself | The character with the ★ tag is you. Move with the arrow keys or WASD, or click (tap on a phone) an empty spot on the floor to walk there. Stand next to a session character and press Space to open its panel; Esc closes it (even while typing). Your position and look are saved in this window. Session characters cannot be moved by hand, since their position is their status. |
...
| 2.5 | Autonomous office: see what helpers do and say, hand-back animation, relay chains between agents, session-to-session conversations, mid-relay intervention | Done |
